...
def get_servicios_ordenados():
    ...
    return (
        ...
        .annotate(_categoria_orden=orden_categoria)
        .order_by("_categoria_orden", "nombre_servicio")
    )
...
def ensure_servicio_categoria_column():
    ...
    try:
        with connection.cursor() as cursor:
            columnas = [
                col.name.lower()
                for col in connection.introspection.get_table_description(cursor, tabla)
            ]
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.is_bound:
            ...
            if self.initial.get("fecha_reserva") and self.initial.get("hora_reserva"):
                ...
                fecha_inicial, hora_inicial = self._descomponer_datetime(
                    self.initial["fecha_hora_inicio"]
                )
            elif getattr(self.instance, "fecha_hora_inicio", None):
                fecha_inicial, hora_inicial = self._descomponer_datetime(
                    self.instance.fecha_hora_inicio
                )
