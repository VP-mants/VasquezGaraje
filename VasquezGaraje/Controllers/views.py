...
from Models.models import (Cliente, Insumo, Reserva, ReservaInsumo, Servicio,
                           Vehiculo)
...
def admin_dashboard(request):
        ...
        reservas_detalle.append(
            {
                ...
                "servicio": getattr(
                    reserva.servicio, "nombre_servicio", str(reserva.servicio)
                ),
                "inicio": inicio_local,
                "fin": fin_local,
    ...
    ingresos_estimados_mes = sum(
        [r.servicio.duracion_servicio * 10 for r in reservas_mes]
    )
    ...
    patentes_anteriores = set(
        Reserva.objects.filter(fecha_hora_inicio__lt=fecha_limite).values_list(
            "patente", flat=True
        )
    )
...
def admin_perfil(request):
    ...
    conteo_por_estado = Reserva.objects.values("estado_reserva").annotate(
        total=Count("estado_reserva")
    )
    ...
    clientes_activos_30 = (
        Cliente.objects.filter(
            reserva__fecha_hora_inicio__gte=ahora - timedelta(days=30)
        )
        .distinct()
        .count()
    )
    ...
    umbral_bajo = 5
    insumos_bajos = Insumo.objects.filter(cantidad__lte=umbral_bajo).order_by(
        "cantidad", "nombre"
    )
        ...
        if target_id and accion:
            ...
            if accion == "make_admin":
                if cliente_obj.es_admin:
                    messages.info(
                        request, f"{cliente_obj.nombre_cliente} ya es administrador."
                    )
                else:
                    cliente_obj.es_admin = True
                    cliente_obj.save()
                    messages.success(
                        request, f"{cliente_obj.nombre_cliente} ahora es administrador."
                    )
            elif accion == "remove_admin":
                if cliente_obj.pk == usuario.pk:
                    messages.warning(
                        request,
                        "No puedes quitar tu propio rol de administrador desde aquí.",
                    )
                elif (
                    not Cliente.objects.filter(es_admin=True)
                    .exclude(pk=cliente_obj.pk)
                    .exists()
                ):
                    messages.error(
                        request, "Debe existir al menos un administrador activo."
                    )
                else:
                    cliente_obj.es_admin = False
                    cliente_obj.save()
                    messages.success(
                        request, f"{cliente_obj.nombre_cliente} ya no es administrador."
                    )
...
def admin_editar_reserva(request, id):
    ...
    class ReservaInsumoForm(forms.Form):
        ...
        cantidad = forms.IntegerField(
            ...
            widget=forms.NumberInput(
                attrs={"class": "admin-insumo-qty", "min": 0, "step": 1}
            ),
        )
    ...
    ReservaInsumoFormSet = forms.formset_factory(
        ReservaInsumoForm, extra=1, can_delete=True
    )
    ...
    if request.method == "POST":
        ...
        if form_es_valido and formset_es_valido:
            existentes_cantidades = {
                relacion.insumo_id: relacion.cantidad_utilizada
                for relacion in relaciones_existentes
            }
            existentes_objetos = {
                relacion.insumo_id: relacion for relacion in relaciones_existentes
            }
            ...
            if not error_stock:
                with transaction.atomic():
                    ...
                    insumos_bloqueados = {
                        obj.pk: obj
                        for obj in Insumo.objects.select_for_update().filter(
                            pk__in=ids_todos
                        )
                    }
                    ...
                    for insumo_id in ids_todos:
                        ...
                        if delta != 0:
                            ...
                            insumo_obj.save(
                                update_fields=["cantidad", "fecha_actualizacion"]
                            )
...
def editar_reserva(request, id):
    ...
    reservas_existentes = Reserva.objects.exclude(pk=reserva.pk).exclude(
    ...
    if request.method == "POST" and form.is_valid():
        ...
        try:
            ...
            if inicio_local < min_datetime_obj:
                _add_slot_error(
                    "No puedes seleccionar un horario en el pasado.", include_date=True
                )
                ...
                if not (HORARIO_APERTURA <= hora_inicio <= HORARIO_CIERRE):
                    ...
                    for r in reservas_existentes:
                        ...
                        if (
                            nueva_inicio < fin_existente
                            and nueva_fin > inicio_existente
                        ):
                            solapada = True
                            break
                    if solapada:
                        _add_slot_error(
                            "Ya existe una reserva en ese horario o se solapa con otra. Elige otro horario."
                        )
                    else:
                        nueva_reserva.save()
                        ...
                        messages.success(
                            request,
                            "Reserva actualizada correctamente. Se ha enviado un correo de confirmación.",
                        )
                        return redirect("ver_perfil")
...
def cancelar_reserva(request, id):
    ...
    if reserva.estado_reserva not in ["Pendiente", "Confirmado"]:
        messages.error(
            request, "Solo puedes cancelar reservas pendientes o confirmadas."
        )
        return redirect("ver_perfil")
...
def cambiar_contrasena(request):
    ...
    return render(
        request, "cambiar_contrasena.html", {"form": form, "usuario": usuario}
    )
...
def home(request):
    ...
    for nombre in nombres_servicios:
        ...
        descripcion = (
            servicio.descripcion_servicio.strip()
            if servicio and servicio.descripcion_servicio
            else ""
        )
        if not descripcion:
...
def agendar_servicio(request):
    ...
    cliente_id = request.session["cliente_id"]
    servicios = get_servicios_ordenados()
    reservas_existentes = Reserva.objects.exclude(
        estado_reserva="Cancelado"
    ...
    if request.method == "POST" and form.is_valid():
        ...
        try:
            ...
            if inicio_local < min_datetime_obj:
                _add_slot_error(
                    "No puedes agendar en un horario anterior al disponible.",
                    include_date=True,
                )
            elif not _es_intervalo_media_hora(inicio_local):
                _add_slot_error("Selecciona horarios en bloques de 30 minutos.")
                ...
                if not (HORARIO_APERTURA <= hora_inicio <= HORARIO_CIERRE):
                    ...
                    if solapada:
                        ...
                        messages.success(
                            request,
                            "Reserva realizada exitosamente. Se ha enviado un correo de confirmación.",
                        )
                        return redirect("ver_perfil")

    reservas_agendadas = []
    reservas_eventos = []
    reservas_futuras = reservas_existentes.filter(
    ...
    for r in reservas_futuras:
        ...
        reservas_agendadas.append(
            {
                "fecha": inicio_local,
                "fecha_label": date_format(inicio_local, r"l d \d\e F"),
